...
% Last Modified by GUIDE v2.5 16-Mar-2015 14:02:38
...
guidata(hObject,handles);
set(handles.statusText,'String','Running');
...
if exist(imPath,'dir')==7
    ...
    set(handles.statusText,'String','The given path does not exist');
...
if nIm>0
    ...
    set(handles.imSelPopUpMenu,'String',vertcat(strImName));
    guidata(hObject,handles);
    ...
    plot(handles.dose(handles.dose~=0),handles.meanRough(handles.dose~=0),'xb');
    xlabel('Dose');
    ylabel('Mean Roughness');
    grid on;
    if handles.dose(val)~=0
        hold on;
        plot(handles.dose(val),handles.meanRough(val),'xr');
        hold off;
    end
